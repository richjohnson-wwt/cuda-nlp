...
int main() {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (auto& v : embedded) input_flat.insert(input_flat.end(), v.begin(), v.end());
    ...
    // ----- Cleanup -----
    cudaFree(d_input); cudaFree(d_wq); cudaFree(d_wk);
    cudaFree(d_wv); cudaFree(d_output);
    cudaFree(d_wout); cudaFree(d_logits);
