...
void self_attention_forward(
    ...
    if (err != cudaSuccess) { std::cerr << "cudaMalloc d_Q failed\n"; std::exit(1); }

    err = cudaMalloc(&d_K, mat_bytes);
    if (err != cudaSuccess) { std::cerr << "cudaMalloc d_K failed\n"; std::exit(1); }

    err = cudaMalloc(&d_V, mat_bytes);
    if (err != cudaSuccess) { std::cerr << "cudaMalloc d_V failed\n"; std::exit(1); }

    err = cudaMalloc(&d_scores, score_bytes);
    if (err != cudaSuccess) { std::cerr << "cudaMalloc d_scores failed\n"; std::exit(1); }

    err = cudaMalloc(&d_attention, score_bytes);
    if (err != cudaSuccess) { std::cerr << "cudaMalloc d_attention failed\n"; std::exit(1); }
    ...
    cudaFree(d_scores); cudaFree(d_attention);
}